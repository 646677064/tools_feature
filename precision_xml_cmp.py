...
def GoodEval(resultfile,Anotations_Dir,SKUfile,test_filelist,cachedir):
    ...
    #     if os.path.splitext(file_comp4)[1] == '.txt':
    #         basename = os.path.splitext(file_comp4)[0]
def precision_xml_cmp(JPEG_dir,Anotations_Dir,right_Anotations_Dir,SKUfile,cachedir,threhold=293):
    with open(SKUfile,"r") as SKUfile_f:
        SKUfile_lines = SKUfile_f.readlines()
    labelmap={}
    labellist=[]
    ...
    if not os.path.isdir(cachedir):
        os.mkdir(cachedir)
    result_filelist= os.listdir(JPEG_dir)
    cachefilename="cache_result.pkl"
    cachefile = os.path.join(cachedir, cachefilename)
    if not os.path.isfile(cachefile):
        # load annots
        gt_obj = {}
        for i_file,test_file in enumerate(result_filelist):
            if os.path.splitext(test_file)[1] !=".jpg":
                continue
            temp_file=os.path.splitext(test_file)[0]+".xml"
            #print Anotations_Dir+temp_file
            if os.path.exists(Anotations_Dir+temp_file)==False:
                continue
            #print test_file
            gt_obj[temp_file]=parse_rec(Anotations_Dir+temp_file)
            print len(gt_obj[temp_file])
            if i_file % 50 == 0:
                print 'Reading annotation for {:d}/{:d}'.format(
                    i_file + 1, len(result_filelist))
        # save
        print 'Saving cached annotations to {:s}'.format(cachefile)
        with open(cachefile, 'w') as f:
            cPickle.dump(gt_obj, f)
    ...
    cachefilename_right="cache_right.pkl"
    cachefile_right = os.path.join(cachedir, cachefilename_right)
    if not os.path.isfile(cachefile_right):
        # load annots
        gt_obj_right = {}
        #result_filelist_right= os.listdir(right_Anotations_Dir)
        for i_file,test_file in enumerate(result_filelist):
            if os.path.splitext(test_file)[1] !=".jpg":
                continue
            temp_file=os.path.splitext(test_file)[0]+".xml"
            if os.path.exists(right_Anotations_Dir+temp_file)==False:
                continue
            gt_obj_right[temp_file]=parse_rec(right_Anotations_Dir+temp_file)
            if i_file % 50 == 0:
                print 'Reading annotation for {:d}/{:d}'.format(
                    i_file + 1, len(result_filelist))
        # save
        print 'Saving cached annotations to {:s}'.format(cachefile)
        with open(cachefile_right, 'w') as f:
            cPickle.dump(gt_obj_right, f)
    else:
        # load
        with open(cachefile_right, 'r') as f:
            gt_obj_right = cPickle.load(f)
    ...
    tianluo_dic={}
    for temp_file,obj_struc_right in gt_obj_right.items():
        tianluo_dic[temp_file] = 0
        tianluo_dic[temp_file+'_sum_'] = 0
        #obj_struc_right=gt_obj_right[temp_file]
    #for i,obj_struc_right in enumerate(gt_obj):
        for j,objcts_right in enumerate(obj_struc_right):
            tmp_name = objcts_right['name']
            tmp_name=tmp_name.lower() #tmp_name.find('other')<0 and
            if tmp_name.find('other')>=0:
                #print temp_file,"===== ",tmp_name
                continue
            if labelmap[objcts_right['name']]>=threhold:
                #print temp_file,"===== ",tmp_name
                continue
            ...
            #     continue
            if  objcts_right['name'] != 'Others' :
                #print objcts_right['name']
                rsum = rsum + 1
                tianluo_dic[temp_file+'_sum_']+=1
                #print temp_file,objcts_right['name']
                label_recall_all[labelmap[objcts_right['name']]-1] +=1
                #for i_Nametxt, indexname in enumerate(Nametxt):
                if gt_obj.has_key(temp_file) ==False:
                    continue
                for i_Nametxt, indexname in enumerate(gt_obj[temp_file]):
                    #if temp_file == indexname  and labelIDtxt[i_Nametxt] == labelmap[objcts_right['name']]:
                    if indexname['name'] == objcts_right['name']:
                        areaRate = IOU(indexname['bbox'][0],indexname['bbox'][1],indexname['bbox'][2],indexname['bbox'][3],
                                       objcts_right['bbox'][0],objcts_right['bbox'][1],objcts_right['bbox'][2],objcts_right['bbox'][3])
                        #print "areaRate= ",areaRate
                        if areaRate >= 0.5:
                            r = r +1
                            label_recall_count[labelmap[objcts_right['name']]-1] +=1
    ...
    tianluo_psum = 0
    #pic_tianluo = {}
    for i_pNametxt, objcts_sturct in gt_obj.items():
    #for i_pNametxt, pindexname in enumerate(Nametxt):
        every_pic_tianluo = 0
        tianluo_psum = 0
        for j,objcts in enumerate(objcts_sturct):
            ...
            if objcts['name'] !='Others':
                ...
                label_precision_all[labelmap[objcts['name']]-1] +=1 #labelmap[objcts_right['name']]-1

                if gt_obj_right.has_key(i_pNametxt) ==False:
                    continue
                for j_p,objcts_p in enumerate(gt_obj_right[i_pNametxt]):
                    #if labelmap[objcts_p["name"]] == labelIDtxt[i_pNametxt]:
                    areaRate_p = IOU(objcts['bbox'][0],objcts['bbox'][1],objcts['bbox'][2],objcts['bbox'][3],
                                objcts_p['bbox'][0],objcts_p['bbox'][1],objcts_p['bbox'][2],objcts_p['bbox'][3])
                    if areaRate_p >= 0.5:
